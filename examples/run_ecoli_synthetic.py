...
def run_causal_discovery(superstructure, partition, df, G_star, screen, run_serial=True, full_cand_set =False):
    ...
    #     print(s[0].shape[0])
    #     start_sub = time.time()
    #     result = _local_structure_learn(s)
    #     results.append(result)
    #     print("Subproblem takes {} (s)".format(time.time()-start_sub))
...
def run():
    ...
    df = get_data_from_graph(nodes, edges, nsamples=int(1e2), iv_samples=0, bias=None, var=None)[-1]
    superstructure = artificial_superstructure(G_star, frac_extraneous=0.1)
    
    print("Start Mod partitioning...")
    start = time.time()
    init_partition = modularity_partition(superstructure, resolution=5, cutoff=20, best_n=20)
    ...
    ss, sp_h = run_causal_discovery(superstructure, init_partition, df,  G_star, screen=True, run_serial=False)
