...
def run_causal_discovery(superstructure, partition, df, G_star, screen, run_serial=True, full_cand_set =False):
    ...
    if screen:
        est_graph_partition, _ = screen_projections(partition, results)
    else:
        est_graph_partition, _ = fusion(superstructure, partition, results, data_obs, full_cand_set=full_cand_set)
...
def run():
    ...
    df = get_data_from_graph(nodes, edges, nsamples=int(1e5), iv_samples=0, bias=None, var=None)[-1]
    ...
    ss, sp_h = run_causal_discovery(superstructure, init_partition, df,  G_star, screen=False)
    ...
    _, sp_c = run_causal_discovery(superstructure, causal_partition, df, G_star, run_serial=False, screen=True)
    ...
    _, sp_c = run_causal_discovery(superstructure, ec_partition, df, G_star, run_serial=False, screen=True)
    
        
    pef_partition = PEF_partition(df)
   # vis("pef_part", pef_partition, G_star)
    print("PEF partition")
    _, sp_c = run_causal_discovery(superstructure, pef_partition, df, G_star, run_serial=False, full_cand_set=True, screen=False)
