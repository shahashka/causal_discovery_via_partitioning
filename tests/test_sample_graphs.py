...
def sample_and_check_sanity(graph_type, n, p, k, ncommunities, alpha, collider_weight, nsamples, outdir):
    ...
    (arcs, nodes, _, _), df = get_data_from_graph( nodes, list(tiled_net.edges()),
                                                  nsamples=nsamples, iv_samples=0, save=False, 
                                                  outdir=outdir)
    ...
    num_wrong = _check_superstructure(superstructure, nx.adjacency_matrix(tiled_net, nodelist=np.arange(len(nodes))))
...
if __name__ == '__main__':
    ...
    fig,axs = plt.subplots(len(p_range),len(k_range), figsize=(10,10), sharex = True, sharey=True)
        ...
        for j,k in enumerate(k_range):
            ...
            for g in graph_types:
                ...
                for n in range(niters):
                    ss_metric, collider_metric = sample_and_check_sanity(g, n=10, p=p, k=k, ncommunities=5, 
                                        alpha=0.1, collider_weight=10, nsamples=int(1e6),
                                        outdir="./datasets/base_case_sweep/")
                    metrics[n] = ss_metric
                ss_metrics_by_graph_type.append(metrics)

                axs[i][j].violinplot(ss_metrics_by_graph_type, showmeans=True, showmedians=False)
                axs[i][j].set_xticks(np.arange(1,len(graph_types)+1),
                            labels=graph_types, rotation=45)
