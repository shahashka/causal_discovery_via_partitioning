...
def single_trial_localize_errors(
    num_nodes=50, base_partition=heirarchical_partition, verbose=False
):
    outdir = "./"
...
def multiple_trial_localize_errors(
    num_trials=10,
    num_nodes=50,
    metric="overall_dist",
    base_partition=heirarchical_partition,
):
    ...
    for _ in range(num_trials):
        ...
        ) = single_trial_localize_errors(
            num_nodes=num_nodes, base_partition=heirarchical_partition, verbose=False
        )
    ...
    # Plot all metrics
    if metric == "all":
        for metric in score_outputs:
            causal_list = _extract_target_metric(
                causal_error_distances, target_metric=metric
            )
            serial_list = _extract_target_metric(
                serial_error_distances, target_metric=metric
            )
            y_label = metric
            titlestring = (
                f"Results over {num_trials} independent trials \n"
                f"Hierarchical network with {num_nodes} nodes \n"
                f"Disjoint partition via {base_partition.__name__} \n"
                f"Trials with errors: Causal partition has {len(causal_list)}, Serial partition has {len(serial_list)} "
            )
            distance_to_boundary_violinplot(
                causal_list, serial_list, y_label, titlestring
            )
    # Plot single metric
    else:
        causal_list = _extract_target_metric(causal_error_distances)
        serial_list = _extract_target_metric(serial_error_distances)
        y_label = metric
        titlestring = (
            f"Results over {num_trials} independent trials \n"
            f"Disjoint partition via {base_partition.__name__} \n"
            f"Causal partition has {len(causal_list)} trials with errors \n"
            f"Serial partition has {len(serial_list)} trials with errors."
        )
        distance_to_boundary_violinplot(causal_list, serial_list, y_label, titlestring)
    return


def distance_to_boundary_violinplot(causal_list, serial_list, y_label, titlestring):
...
multiple_trial_localize_errors(
    num_trials=50, num_nodes=200, base_partition=heirarchical_partition, metric="all"
)
