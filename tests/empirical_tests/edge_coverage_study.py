...
from cd_v_partition.overlapping_partition import rand_edge_cover_partition, expansive_causal_partition, modularity_partition, partition_problem
...
def run_study():
    ...
    data = [scores_serial[:,:,0], scores_edge_cover[:,:,0], scores_causal_partition[:,:,0], scores_mod_partition[:,:,0]] 
    ...
    sns.boxplot(data=df, x='samples', y='value', hue='variable', order=x_order, hue_order=labels, ax=axs[0])
    axs[0].set_xlabel("Fraction of Extraneous edges")
    axs[0].set_ylabel("TPR")
    
    
    data = [scores_serial[:,:,1], scores_edge_cover[:,:,1], scores_causal_partition[:,:,1], scores_mod_partition[:,:,1]] 
    data = [np.reshape(d, num_repeats*len(ss_density)) for d in data]
    labels = [ 'serial', 'edge_cover', 'expansive_causal', 'mod']
    df = pd.DataFrame(data=np.column_stack(data), columns=labels)
    df['samples'] = np.repeat(ss_density, num_repeats)
    df = df.melt(id_vars='samples', value_vars=labels)
    x_order = np.unique(df['samples'])
    sns.boxplot(data=df, x='samples', y='value', hue='variable', order=x_order, hue_order=labels, ax=axs[1])
    ...
    plt.tight_layout()
    plt.savefig(
        "./tests/empirical_tests/edge_coverage_study.png"
    )
