...
# Create G_star
(sf_arcs, _, _, _), df = get_random_graph_data(
    graph_type="scale_free", num_nodes=50, nsamples=int(1e4), iv_samples=0, p=0.2, k=1
)
print(len(sf_arcs))
...
colors = dict(
    zip(
        np.arange(50),
        [color_map[0] if node < 40 else color_map[1] for node in np.arange(50)],
    )
)
...
edge_colors = [
    "red"
    if (edge[0] < 40 and edge[1] < 40)
    and (full_adj[edge[0], edge[1]] != inner_adj[edge[0], edge[1]])
    else "black"
    for edge in edges
]
...
delta_causality(
    full_adj[sub_nodes][:, sub_nodes], inner_adj, sf_adj[sub_nodes][:, sub_nodes]
)
