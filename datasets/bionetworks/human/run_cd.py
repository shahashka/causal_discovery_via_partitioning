...
def load_data_and_superstructure(data_file, superstructure_file='random_corr_coef.txt', use_corr=True, genes_are_symbols=False):
    radbio_data = pd.read_csv(data_file)
    ...
    # If the dataset contains gene sumbols
        ...
        data_symbol_ids_w_dose = data_symbol_ids + ["Dose"]
...
def get_de_genes(rad_level, dir, padj_value=0.05):
    level_to_file_map = {"0.168Gy" : "deseq2_0.168_W1vs0_by_week_results.csv",
                         "0.336Gy": "deseq2_0.336_W2vs0_by_week_results.csv",
                         "0.504Gy": "deseq2_0.504_W3vs0_by_week_results.csv",
                         "1.68Gy": "deseq2_1.68_W1vs0_by_week_results.csv",
                         "3.36Gy": "deseq2_3.36_W2vs0_by_week_results.csv",
                         "5.04Gy": "deseq2_5.04_W3vs0_by_week_results.csv",
                         "16.8Gy": "deseq2_16.8_W1vs0_by_week_results.csv",
                         "33.6Gy": "deseq2_33.6_W2vs0_by_week_results.csv",
                         "50.4Gy": "deseq2_50.4_W3vs0_by_week_results.csv",
                         "168.0Gy": "deseq2_168_W1vs0_by_week_results.csv",
                         "336.0Gy": "deseq2_336_W2vs0_by_week_results.csv",
                         "504.0Gy": "deseq2_504_W3vs0_by_week_results.csv",
                         "672.0Gy": "deseq2_672_W2vs0_by_week_results.csv",
                         "1008.0Gy": "deseq2_1008_W3vs0_by_week_results.csv"}
    de_df = pd.read_csv(f"{dir}/{level_to_file_map[rad_level]}", header=0)
...
def run_causal_discovery_w_partition(data, superstructure, genes=None, level='0.Gy', partition=None,
...
if __name__=="__main__":
    ...
    data_file = "rnaseq_lucid/data.csv" #"de_radbio_data.csv"
    de_dir = "rnaseq_lucid/deseq2"
    important_genes = ["PTGS2",
                        "DEPP1",
                        "MT2A",
                        "G0S2",
                        "SERPINB2",
    ...
    for i, level in enumerate(radiation_levels):
        ...
        if level != "0.0Gy":
            genes = get_de_genes(rad_level=level, dir=de_dir)
        else:
            genes = list(data_cd.columns)
            genes.remove('target')

        graph, partition = run_causal_discovery_w_partition(data_cd, ss, genes=genes,level=level, partition=partition, partition_fn="modularity")
        ...
        nx.write_gexf(graph, f"dag{save_suffix}_{level}.gexf")
        graphs[level] = graph
        print(f"Number of edges: {len(graph.edges)}")
        
        ego_graph = get_ego_graphs(graph, important_genes)
        nx.write_gexf(ego_graph,  f"ego_dag{save_suffix}_{level}.gexf")
        nx.write_edgelist(ego_graph, f"ego_dag{save_suffix}_{level}.csv")
