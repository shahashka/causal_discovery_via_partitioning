...
class MissingPartitionsError(Exception):
    """Raised when a partitioning algorithm does not return any legal partitions."""
...
def modularity_partition(
    adj_mat: np.ndarray, resolution: int = 1, cutoff: int = 1, best_n: int | None = None
):
    ...
    Args:
        ...
        best_n (int | None): upper limit on number of communities before termination
...
def hierarchical_partition(adj_mat: np.ndarray, max_community_size: float = 0.5):
    ...
    Args:
        adj_mat (np.ndarray): the adjacency matrix for the superstructure
        max_community_szie (float): controls the size of the largest community in
            returned partition
        See networkx documentation for more.
...
def oslom_algorithm(
    ...
    Args:
        nodes (): ...
        dat_file (Path | str): The directory containing the *.dat file which holds the edges of the
