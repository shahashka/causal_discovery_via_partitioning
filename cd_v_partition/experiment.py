    ...
    def run_concurrent(
        ...
        with ProcessPoolExecutor(self.workers) as executor:
            ...
            for fut in as_completed(futures):
                ...
                outdir = Path(f"{cfg.experiment_id}/{date}/{alg}/spec_{spec_id}/trial_{trial}/")
    ...
    def run_serial(self, cfg: SimulationConfig, random_state: RandomState | int | None = None):
        ...
        for trial in range(cfg.graph_per_spec):
            ...
            for spec_id, spec in enumerate(cfg):
                ...
                outdir = Path(f"{cfg.experiment_id}/{date}/{spec.partition_fn}/spec_{spec_id}/trial_{trial}/")
                if not outdir.exists():
                    outdir.mkdir(parents=True)
...
# # TODO think about how we want to do this...
# def read_visualize(date:Any, eval_algs: list[str], num_trials: int, 
#                    save_sweep_param: str, save_sweep_values:Any):
#     def sns_vis(cfg, results, ax, ind, label):
#         data = [ s[:,:,ind] for s in results]
#         data = [np.reshape(d, cfg.graph_per_spec * len(cfg[save_sweep_param])) for d in data]
#         df = pd.DataFrame(data=np.column_stack(data), columns=eval_algs)
#         df["samples"] = np.repeat(
#             [save_sweep_values], cfg.graph_per_spec, axis=0
...
#     # Save score matrices
#     for s, l in zip(results_algs, cfg.partition_fn):
#         np.savetxt(
