...
if __name__ == "__main__":
    ...
    sim_cfg = SimulationConfig(graph_per_spec=1,
                               experiment_id=dir,
                               partition_fn=['modularity', 'edge_cover', 'expansive_causal'],
                               num_samples=[int(1e4)],
                               graph_kind="ecoli",
                               graph_load_path=[f"{ecoli_data_dir}/net_{i}.txt" for i in range(2,3)], 
                               causal_learn_fn=['GES'],#["PC","RFCI", "GES","NOTEARS"], 
                               merge_fn=["screen"],
                               save_vis=True
                               )
    ...
    for cd_alg in sim_cfg.causal_learn_fn:
        vis_experiment(6, dir, sim_cfg.partition_fn + sim_cfg_pef.partition_fn,
                       cd_alg, sim_cfg.graph_per_spec, "ecoli_net", 
