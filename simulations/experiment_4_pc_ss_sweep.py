# Experiment 4: two community, scale free, num samples 1e4, num_nodes=50, 
...
def run_samples(experiment_dir, num_repeats, alpha_range, nthreads=16, screen=False):
    nsamples = 1e4
    ...
    for i in range(num_repeats):
        ...
        for j,alpha in enumerate(alpha_range):
            ...
            # Find superstructure 
            superstructure, _ = pc(df, alpha=alpha, num_cores=nthreads)
    ...
    df['samples'] = np.repeat(alpha_range, num_repeats)
...
if __name__ == "__main__":
    run_samples("./simulations/experiment_4/", nthreads=16, num_repeats=10, alpha_range=np.arange(0,0.8,0.1), screen=False)
    run_samples("./simulations/experiment_4/", nthreads=16, num_repeats=10, alpha_range=np.arange(0,0.8,0.1), screen=True)
