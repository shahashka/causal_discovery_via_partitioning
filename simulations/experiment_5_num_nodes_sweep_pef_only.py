...
def run_nnodes(experiment_dir, num_repeats, nnodes_range, nthreads=16, screen=False):
    ...
    for i in range(num_repeats):
        for j,nnodes in enumerate(nnodes_range):
            ...
            ss, sp, ts, tp = run_causal_discovery(dir_name, "pef",superstructure, partition, df, G_star, nthreads=nthreads,screen=screen, full_cand_set=True, run_serial=True)
    ...
    tpr_ind = -3
    data = [scores_serial[:,:,tpr_ind], scores_pef[:,:,tpr_ind]] 
    ...
    fpr_ind = -2
    data = [scores_serial[:,:,fpr_ind], scores_pef[:,:,fpr_ind]] 
    data = [np.reshape(d, num_repeats*len(nnodes_range)) for d in data]
    labels = [ 'serial', 'pef'] 
    df = pd.DataFrame(data=np.column_stack(data), columns=labels)
    df['samples'] = np.repeat([nnodes_range], num_repeats, axis=0).flatten() 
    ...
    shd_ind = 0
    data = [scores_serial[:,:,shd_ind], scores_pef[:,:,shd_ind]] 
    data = [np.reshape(d, num_repeats*len(nnodes_range)) for d in data]
    labels = [ 'serial', 'pef'] 
    df = pd.DataFrame(data=np.column_stack(data), columns=labels)
    df['samples'] = np.repeat([nnodes_range], num_repeats, axis=0).flatten() 
    ...
    time_ind = -1
    data = [scores_serial[:,:,time_ind], scores_pef[:,:,time_ind]]
    data = [np.reshape(d, num_repeats*len(nnodes_range)) for d in data]
    labels = [ 'serial', 'pef'] 
    df = pd.DataFrame(data=np.column_stack(data), columns=labels)
    df['samples'] = np.repeat([nnodes_range], num_repeats, axis=0).flatten() 
...
if __name__ == "__main__":
    ...
    run_nnodes("./simulations/experiment_5_pef_only/", nthreads=16, num_repeats=10, nnodes_range=[10**i for i in np.arange(1,5)], screen=False)
    run_nnodes("./simulations/experiment_5_pef_only/", nthreads=16, num_repeats=10, nnodes_range=[10**i for i in np.arange(1,5)], screen=True)
