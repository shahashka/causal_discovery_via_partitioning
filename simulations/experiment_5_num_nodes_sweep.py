...
def run_causal_discovery(
    ...
    print(
        "Time for partitioned run with {} nodes is {} (s)".format(
            len(est_graph_partition.nodes()), time_partition
        )
    )
...
def run_nnodes_alg(
    ...
    for i in range(num_repeats):
        for j, nnodes in enumerate(nnodes_range):
            ...
            if algorithm == "serial":
                ...
                print(
                    "Time for serial run with {} nodes is {} (s)".format(
                        est_graph_serial.shape[0], time_serial
                    )
                )
                ...
                start = time.time()
                mod_partition = modularity_partition(
    ...
    plt.tight_layout()
    plot_dir = (
        "./{}/screen_projections/{}".format(experiment_dir, algorithm)
        if screen
        else "./{}/fusion/{}".format(experiment_dir, algorithm)
    )
